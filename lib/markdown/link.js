  ...
  md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    if (hrefIndex >= 0) {
      ...
      if (isExternal || isStaticFile) {
        ...
        let txt = tokens[idx+1].content;
        if (hashMatch && txt && hashMatch[1] === txt) {
          addAttr(token, "class", "ref-link");
        }
      }
    }
...
function resolveGithubFileURL(env, href) {
  ...
  let dir = env.dir ? encodeURIComponent(env.dir) + '/' : "";
  let githubRepoBaseURL = `https://github.com/${repoUrl}/blob/${branch}/${dir}`;
